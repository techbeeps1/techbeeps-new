...
export default function ServiceTemplate({ service }: ServiceTemplateProps) {
  ...
  return (
    <>
            ...
            {offeringsSection.items.map((item, idx) => {
              const CardWrapper = item.url
                ? ({ children }: { children: React.ReactNode }) => (
                  <Link href={item.url!} className="group relative rounded-[15px] p-[1px] transition-all duration-500 ease-out hover:-translate-y-3 hover:shadow-[0_30px_60px_-15px_rgba(80,76,255,0.4)] cursor-pointer overflow-hidden block">
                    {children}
                  </Link>
                )
                : ({ children }: { children: React.ReactNode }) => (
                  <div className="group relative rounded-[15px] p-[1px] transition-all duration-500 ease-out hover:-translate-y-3 hover:shadow-[0_30px_60px_-15px_rgba(80,76,255,0.4)] overflow-hidden">
                    {children}
                  </div>
                );
              return (
      ...
      <section className="py-25 border-y-1 border-white/12">
